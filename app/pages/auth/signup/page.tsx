...
const SignUpPage: React.FC = () => {
  ...
  const [isOtpScreen, setIsOtpScreen] = useState(false); // State to toggle OTP UI
  const router = useRouter();

  const handleSignUp = () => {
    console.log("Sign Up with", { username, email, password, confirmPassword });
    // Simulate transitioning to OTP screen after sign-up
    setIsOtpScreen(true);
  };
      ...
      <section className="bg-gradient-to-br from-[#FFE7C7] to-purple-200 shadow-2xl rounded-2xl flex flex-col md:flex-row w-full max-w-4xl p-3 md:p-6 transition-transform duration-500 md:hover:scale-105">
        ...
        {isOtpScreen ? (
          <div className="flex-1 flex flex-col justify-center px-3">
            ...
            <button
              className="w-full mt-4 py-2 px-3 bg-orange-400 hover:bg-orange-600 text-white rounded-[10px] shadow-md transition-all duration-500 focus:ring-2 focus:ring-purple-300"
